...
const float RANGE_DETECTION_LOAD = 100;
const int MAX_SAFETY_CURRENT = 650;
...
bool jump(float RPMGOAL) {
  ...
  while ((actualLoad + RANGE_DETECTION_LOAD) > lastLoad) {
    ...
    CourrantM1_bluetooth = currentdx1;    
    ...
    if ((abs(currentdx1) + abs(currentdx2)) >= MAX_SAFETY_CURRENT) {
